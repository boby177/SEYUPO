...
import com.seyupo.actions.aware.AwareController;
import com.seyupo.barcodereader.BarcodeActivity;
import com.seyupo.SeyupoAccountData;
import com.seyupo.SeyupoConfig;
import com.seyupo.SeyupoLogger;
import com.seyupo.SeyupoUtils;
import com.seyupo.R;
import com.seyupo.exceptions.NoMoreDevicesAllowedException;
import com.seyupo.net.SeyupoWebServices;
import com.seyupo.util.KeyboardStatusDetector;
import com.seyupo.util.KeyboardVisibilityListener;
...
public class SignInActivity extends Activity {
    ...
    public void onResume() {
        SeyupoLogger.d("onResume of SignInActivity");
        super.onResume();
    ...
    @Override
    public void onPause() {
        SeyupoLogger.d("onPause of SignInActivity");
        super.onPause();
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        SeyupoLogger.d("onCreate of SignInActivity");
        ...
        keyboard.setVisibilityListener(new KeyboardVisibilityListener() {
            ...
            public void onVisibilityChanged(boolean keyboardVisible) {
                try {
                    ...
                    if (keyboardVisible) {
                        SeyupoLogger.d("key on");

                        params.setMargins(20,0,20,halfHeight);
                    } else {
                        SeyupoLogger.d("key off");

                        params.setMargins(20,0,20,20);
                    }
                    linkSignin.setLayoutParams(params);
                } catch (Exception e) {
                    SeyupoLogger.e("error:"+e.getMessage(),e);
                }
            }
        });
                    ...
                    if (email.length() < 6 || email.length() > 200) {
                        ...
                        if (password.length() < 6 || password.length() > 256) {
                            ...
                            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB)
                                new AddDeviceToAccount().executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, email, password, SeyupoUtils.getDeviceType(ctx));
                            else
                                new AddDeviceToAccount().execute(email, password, SeyupoUtils.getDeviceType(ctx));
                        }
                    }
                }
    ...
    private class AddDeviceToAccount extends AsyncTask<String, Void, Void> {
        ...
        protected Void doInBackground(String... data) {
            try {
                ...
                error = null;
                SeyupoAccountData accountData = SeyupoWebServices.getInstance().registerNewDeviceToAccount(SignInActivity.this, data[0], data[1], data[2]);
                SeyupoConfig.getSeyupoConfig(getApplicationContext()).saveAccount(accountData);
                SeyupoConfig.getSeyupoConfig(getApplication()).registerC2dm();
                SeyupoWebServices.getInstance().sendEvent(getApplication(), SeyupoConfig.ANDROID_SIGN_IN);
                String email=SeyupoWebServices.getInstance().getEmail(getApplicationContext());
                SeyupoConfig.getSeyupoConfig(getApplicationContext()).setEmail(email);
                new Thread() {
                    public void run() {
        ...
        protected void onPostExecute(Void unused) {
            ...
            if (noMoreDeviceError)
                ...
                if (error == null) {
                    ...
                    Intent intent =null;
                    if (SeyupoConfig.getSeyupoConfig(SignInActivity.this).isChromebook()) {
                        intent = new Intent(SignInActivity.this, WelcomeActivity.class);
                        SeyupoConfig.getSeyupoConfig(SignInActivity.this).setProtectReady(true);
                    }else {
